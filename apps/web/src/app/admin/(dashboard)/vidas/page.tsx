...
export default function AdminVidasPage() {
  ...
  useEffect(() => {
    setPageTitle('Vidas', 'Visão consolidada de todas as tenants');
  }, [setPageTitle]);
  ...
  const load = useCallback(async (p: number, q: string, st: string, sub: string, uf: string) => {
    try {
      ...
      const res = await api(`/vidas?${params}`, { tenantId: tenantHeader });
      setPageData(res);
    } catch {
      setPageData(null);
    }
  ...
  }, [page, search, statusFilter, subscriptionFilter, estadoFilter, load]);
  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-ink">Vidas</h1>
            ...
            <tbody>
              {!pageData || pageData.data.length === 0 ? (
                ...
                pageData.data.map((t) => {
                  ...
                  return (
                  <tr key={`${t.tipo}-${t.id}`} className="border-b border-edge/70 last:border-0 transition-colors hover:bg-surface-canvas/40">
